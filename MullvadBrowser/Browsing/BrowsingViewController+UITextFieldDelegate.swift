...
extension BrowsingViewController: UITextFieldDelegate {
	...
	private static let insecureIcon = UIImage(systemName: "lock.slash")
	...
	*/
	func updateEncryptionBt(_ mode: Tab.SecureMode) {
		...
		case .secure, .secureEv:
			icon = Self.secureIcon
			color = .ok

		case .mixed:
			icon = Self.insecureIcon
			color = .gray
		...
		encryptionBt.setImage(icon, for: .normal)
	...
	*/
	private func parseSearch(_ search: String?) -> URL? {
		   ...
		   !search.isEmpty
		{
			...
			if search.caseInsensitiveCompare(URL.aboutMullvadBrowser.absoluteString) == .orderedSame {
